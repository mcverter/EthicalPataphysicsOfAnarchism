...
my $sthPtr = $eDBh->selectEtymRadicalsFromWord($wordID);
	...
	my $view = "<a href=\"GUI.cgi?mode=showEtymRadical&etymRoot=$radicalID\">Explore \"$radical\"</a>";
...
my $sthPtr = $eDBh->selectSemCategoriesFromWord($wordID);
	...
	my $view = "<a href=\"GUI.cgi?mode=showSemCategory&semRoot=$categoryID\">Explore \"$category\"</a>";
...
print DEBUG_LOG "after  selecting words from database\n";
	...
	my $wordID = $rootRef->[0];
	my $word = $rootRef->[1];
	my $view = "<a href=\"GUI.cgi?mode=showWord&word=$wordID\">Explore \"$word\"</a>";
...
sub printSemanticCategoryScreen
{
    ...
    print $cgi->header();
    print $cgi->start_html(
		       -title => "Information for Semantic Category $category"
		       );

    printSemanticCategoryDetails($category);
    print DEBUG_LOG "all details printed\n";
    printAllSemanticCategoryWords($category);
    print $cgi->end_html;

}
sub printAllSemanticCategoryWords
{
    my $category = shift;
    print <<"=====";
      <table border="1">
	  <caption> Words with the Semantic Category $category</caption>
	  <tr><th>Word</th><th>ViewMore</th></tr>
=====
print DEBUG_LOG "about to select words from database\n";
my $sthPtr = $eDBh->selectWordsFromSemanticCategory($category);
print DEBUG_LOG "after  selecting words from database\n";
    my $rootRef;
    while (( $rootRef = $eDBh->getNext($sthPtr))
	   && (@$rootRef))
    ...
    print "</table>";
    print DEBUG_LOG "at end of print semantic category words\n";

}
sub printSemanticCategoryDetails
{
    print <<"=====";
...
=====
&InsertStaticTable::insertWordsWithEtymRoots;
...
=====
InsertStaticTable::insertEtymologicalRoots();
...
{    
    ...
    <select name=semRoot size=10>	        
=====
InsertStaticTable::insertSemanticCategories();
