...
const tiWords = fs
  .readFileSync(__dirname + "/../data/words/TIWords.txt", "utf-8")
  .split(" ");
const otbWords = fs
  .readFileSync(__dirname + "/../data/words/OTBWords.txt", "utf-8")
  .split(" ");
...
for (let w in wordMap) {
  wordMap[w].otb =
    Math.round((10 * wordMap[w].otb * tiWords.length) / otbWords.length) / 10;
