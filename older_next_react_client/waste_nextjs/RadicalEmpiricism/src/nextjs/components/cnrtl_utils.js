...
function installSearchEngine() {
  if (window.external && "AddSearchProvider" in window.external) {
    window.external.AddSearchProvider(
      "http://www.cnrtl.fr/portail/opensearch.xml"
    );
  } else if (window.sidebar && "addSearchEngine" in window.sidebar) {
    window.sidebar.addSearchEngine(
      "http://www.cnrtl.fr/portail/opensearch.src",
      "http://www.cnrtl.fr/portail/opensearch.png",
      "Portail Lexical - CNRTL",
      "Recherche lexicographique dans le TLFi"
    );
  } else {
...
function changeColor(index, select) {
  ...
  if (newclass > 0)
    changeCSS(tlf_classes[newclass], "backgroundColor", tlf_colors[index][0]);
...
function displayOptionBox(parent) {
  ...
  for (x in tlf_colors) {
    ...
    if (value > 0)
      document.optionBoxForm[tlf_colors[x][2]].options[value].selected = true;
  }
  ...
  for (var S = 0; S < nbsheets; S++) {
    ...
    if (rules != null)
      for (var R = 0; R < rules.length; R++)
        if (rules[R].selectorText == theClass) rules[R].style[element] = value;
  }
}
...
function showNavigationMenu(e) {
  ...
  if (str != "") {
    ...
    for (var name in PORTAIL_ACTION)
      content +=
        '<li><a href="' +
        PORTAIL_ACTION[name] +
        str +
...
function playSound(url) {
  ...
  if (element != null) {
    if (
      ...
      element.innerHTML =
        '<object type="audio/basic" data="' +
        url +
        '" width="0" height="0"><param name="src" value="' +
        url +
        '" /><param name="autoplay" value="true" /><param name="autoStart" value="0" /></object>';
    } else {
      if (navigator.appVersion.indexOf("MSIE") != -1)
        element.innerHTML =
          '<object id="WMP" width="0" height="0" classid="CLSID:6BF52A52-394A-11d3-B153-00C04F79FAA6" type="application/x-oleobject"><param name="url" value="' +
          url +
          '"><param name="autostart" value="1" /><param name="showcontrols" value="0" /><param name="showdisplay" value="0" /><param name="showstatusbar" value="0" /><param name="loop" value="0" /></object>';
      else
        element.innerHTML =
          '<object id="WMP" type="application/x-ms-wmp"><param name="url" value="' +
          url +
          '"><param name="autostart" value="1" /><param name="uimode" value="0" /><param name="loop" value="0" /></object>';
    }
  }
}
...
function printPage() {
  ...
  a = window.open(
    "",
    "",
    "scrollbars=yes,toolbar=no,resizable=yes,status=yes,width=800,height=400"
  );
  x = a.document;

  x.open("text/html");
  x.write(
    '<html><head><meta http-equiv="content-language" content="fr" /><meta http-equiv="content-type" content="text/html; charset=utf-8" />'
  );
  x.write(
    '<link type="text/css" rel="stylesheet" media="all" href="/css/portail' +
      version +
      '.css" />'
  );
  x.write(
    '<title>Portail lexical</title></head><body onload="initPortail();"><div id="wrap"><div id="main_content">'
  );
  ...
  x.write(
    '</div></div><script type="text/javascript" src="/js/utilities' +
      version +
      '.js"></script></body></html>'
  );
  x.close();
